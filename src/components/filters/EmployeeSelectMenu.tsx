...
export default function EmployeeSelectMenu({ setSelectedUser, selectedUser }) {
  const [users, setUsers] = React.useState<User[]>([]);

  React.useEffect(() => {
    const fetchUsers = async () => {
      const fetchedUsers = await fetchAllUsers();
      const newUsers = [{ fullName: 'All Employees', id: '' }].concat(fetchedUsers);
      setUsers(newUsers);
    };
  ...
  return (
    <Autocomplete
      ...
      options={users}
      value={selectedUser ? selectedUser : { fullName: 'All Employees', id: '' }}
